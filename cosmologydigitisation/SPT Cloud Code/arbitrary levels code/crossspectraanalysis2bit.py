...
allcmbnoisemaps2bithm = [zeros((mappixelnumber, mappixelnumber)) for i in range(ranks)]
allcmbnoisemaps2bitopt = [zeros((mappixelnumber, mappixelnumber)) for i in range(ranks)]
...
for rank in range(ranks):
    ...
    for row in open(digithmnoisemapname):
        rowvalues = row.split()
        for x in range(mappixelnumber):
            allcmbnoisemaps2bithm[rank][y, x] += float(rowvalues[x])
        y += 1
    ...
    for row in open(digitoptnoisemapname):
        rowvalues = row.split()
        for x in range(mappixelnumber):
            allcmbnoisemaps2bitopt[rank][y, x] += float(rowvalues[x])
        y += 1
...
while i < ranks:
    j = i+1
    while j < ranks:
        kd2hmx, pd2hmx, errd2hmx, hd2hmx = cosm.sriniPowerSpectrum(mapparams, allcmbnoisemaps2bithm[i], allcmbnoisemaps2bithm[j])
        kd2optx, pd2optx, errd2optx, hd2optx = cosm.sriniPowerSpectrum(mapparams, allcmbnoisemaps2bitopt[i], allcmbnoisemaps2bitopt[j])
        kobsx, pobsx, errobsx, hobsx = cosm.sriniPowerSpectrum(mapparams, allcmbnoisemaps[i], allcmbnoisemaps[j])
        crosspd2hm = crosspd2hm + pd2hmx
        crosspd2opt = crosspd2opt + pd2optx
...
crosspd2hm = crosspd2hm / float(norm)
crosspd2hm = crosspd2hm * float(pixelnumber) * float(pixelnumber) / pi
crosspd2opt = crosspd2opt / float(norm)
crosspd2opt = crosspd2opt * float(pixelnumber) * float(pixelnumber) / pi
...
obspow2bithm = 0
obspow2bitopt = 0
while mmi < len(kd2hmx):
    if kd2hmx[mmi] >= 350 and kd2hmx[mmi] <= 1000:
        truepow += p0[mmi]**2.0
        obspow2bithm += crosspd2hm[mmi]**2.0
        obspow2bitopt += crosspd2opt[mmi]**2.0
    mmi += 1

crosspd2hm = crosspd2hm * sqrt(truepow/obspow2bithm)
crosspd2opt = crosspd2opt * sqrt(truepow/obspow2bitopt)

print(mean(crosspd2hm-p0))
print(std(crosspd2hm-p0))
print(mean(crosspd2opt-p0))
print(std(crosspd2opt-p0))


#plot(k0, p0, "b")
plot(kd2hmx, crosspd2hm-p0, "r", label="HM")
plot(kd2optx, crosspd2opt-p0, "b", label="OPT")
xscale("log")
yscale("linear")
xlim((4, 8e3))
legend(fontsize = 15)
title(r"2 Bit X-spectra - true PS", fontsize = 20)
